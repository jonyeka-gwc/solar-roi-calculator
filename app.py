...
To deploy on Streamlit Cloud:
    ...
    6. Create results_schema_min.json with results section copy
    7. Go to share.streamlit.io and connect your repo
"""
...
# Load results schema for data-driven copy
try:
    with open("results_schema_min.json", "r") as f:
        schema = json.load(f)
except FileNotFoundError:
    st.error("results_schema_min.json not found. Please ensure it's in the same directory as app.py")
    schema = None

# Helper function to replace placeholders in text templates
def render_with_placeholders(template, data):
...
# DEVELOPER NOTE: SEG rate constants
# 4p/kWh represents the UK government's minimum guaranteed SEG rate
# Used as conservative default when supplier is unknown or not selected
DEFAULT_SEG_RATE = 4.0  # Minimum guaranteed SEG rate for unknown suppliers
...
# DEVELOPER NOTE: EV impact model - relative multipliers applied to base usage
# This approach scales proportionally with household size rather than fixed additions
# Future enhancement: variable mileage inputs, smart charging profiles, vehicle efficiency
# Current multipliers: Low +50%, Medium +60%, High +70%, Actual +60%
# Based on typical UK EV home charging: 7,000-10,000 miles/year ≈ 2,000-3,000 kWh

# Define base annual usage values (without EV)
BASE_USAGE = {
    'low': 1800,
    'medium': 2700,
    'high': 4100
...
# EV multiplier factors (applied when has_ev == "Yes")
EV_MULTIPLIERS = {
    'low': 1.50,      # +50% for small homes
    'medium': 1.60,   # +60% for typical homes
    'high': 1.70,     # +70% for large homes
    'actual': 1.60    # +60% for custom input
}
...
with col_roof1:
    ...
    if postcode and postcode.strip() != "":
        ...
        st.markdown(
            f'🗺️ <a href="{maps_url}" target="_blank">View your roof on Google Maps</a>',
...
# ============================================================================
# DEVELOPER NOTE: Usage selection redesigned for MVP testing feedback
# Button-based presets improve scannability and reduce decision fatigue
# Responsive layout: 4 inline (desktop) → 2×2 grid (mobile) at 768px breakpoint
st.markdown("### ⚡ Electricity Usage")
st.markdown("How much electricity does your home use each year?")
st.caption("Choose the option that best matches your home — or select Actual to enter your own usage.")

# Usage selection buttons using columns
col_usage1, col_usage2, col_usage3, col_usage4 = st.columns(4)

with col_usage1:
    usage_low = st.button("💡 Low", use_container_width=True, key="usage_low")
with col_usage2:
    usage_medium = st.button("🏠 Medium", use_container_width=True, key="usage_medium")
with col_usage3:
    usage_high = st.button("⚡ High", use_container_width=True, key="usage_high")
with col_usage4:
    usage_actual = st.button("📊 Actual", use_container_width=True, key="usage_actual")

# DEVELOPER NOTE: Session state manages selected usage type and persists across reruns
# Default to Medium if no selection made yet
if 'usage_type' not in st.session_state:
    st.session_state.usage_type = 'medium'
    st.session_state.annual_usage = BASE_USAGE['medium']

# Update selection based on button clicks
if usage_low:
    st.session_state.usage_type = 'low'
    st.session_state.annual_usage = BASE_USAGE['low']
elif usage_medium:
    st.session_state.usage_type = 'medium'
    st.session_state.annual_usage = BASE_USAGE['medium']
elif usage_high:
    st.session_state.usage_type = 'high'
    st.session_state.annual_usage = BASE_USAGE['high']
elif usage_actual:
    st.session_state.usage_type = 'actual'

st.write("")

# Blue Box #1: non-computational UI element placed directly below usage selector; no link to calculation variables.
# Shows qualitative description only (what this usage level means) - hidden for "Actual" selection
if st.session_state.usage_type == 'low':
    st.info("""
    **💡 Low Usage**  
    Small flat or home with 1-2 people, gas heating and cooking.
    """)
elif st.session_state.usage_type == 'medium':
    st.info("""
    **🏠 Medium Usage**  
    Typical 2-3 bedroom home with 3-4 people, some electric appliances.
    """)
elif st.session_state.usage_type == 'high':
    st.info("""
    **⚡ High Usage**  
    Large home with 4+ people, or extensive use of electric appliances.
    """)

st.write("")

# Additional usage questions - EV question BEFORE usage display
...
with col3:
    has_ev = st.radio(
        ...
        ["No", "Yes"],
        index=0,
        help="EV charging increases electricity usage by ~50-70% depending on household size"
    )
...
# For "Actual" selection, show number input here (between questions and Box #2)
if st.session_state.usage_type == 'actual':
    base_usage = st.number_input(
        "Enter your annual electricity usage (kWh)",
        min_value=1000,
        max_value=20000,
        value=st.session_state.annual_usage if st.session_state.annual_usage not in [BASE_USAGE['low'], BASE_USAGE['medium'], BASE_USAGE['high']] else BASE_USAGE['medium'],
        step=100,
        help="You can find this on your electricity bill or smart meter app"
    )
    st.session_state.annual_usage = base_usage
    st.caption("💡 You can find this on your electricity bill or smart meter app.")
    st.write("")

# Calculate annual usage with EV adjustment applied (all calculations for Box #2)
if st.session_state.usage_type == 'low':
    base_usage = BASE_USAGE['low']
    annual_usage = base_usage * EV_MULTIPLIERS['low'] if has_ev == "Yes" else base_usage
elif st.session_state.usage_type == 'medium':
    base_usage = BASE_USAGE['medium']
    annual_usage = base_usage * EV_MULTIPLIERS['medium'] if has_ev == "Yes" else base_usage
elif st.session_state.usage_type == 'high':
    base_usage = BASE_USAGE['high']
    annual_usage = base_usage * EV_MULTIPLIERS['high'] if has_ev == "Yes" else base_usage
else:  # actual
    # base_usage already set above in the number_input
    annual_usage = base_usage * EV_MULTIPLIERS['actual'] if has_ev == "Yes" else base_usage

# Blue Box #2: quantitative usage summary - always displayed at bottom with calculated total
st.info(f"""
**Approximate annual usage: {int(annual_usage):,} kWh electricity.**
""")

# Show EV impact caption when EV charging is enabled
if has_ev == "Yes":
    st.caption("🔋 Includes extra energy for home EV charging (approx. 50–70% increase).")
...
# ============================================================================
# DEVELOPER NOTE: Tariff section rewritten for conversational clarity ahead of Userberry testing
# Goal: Guide non-technical users through supplier → SEG → day/night rates like a conversation
# Explicit instructions reduce confusion and build confidence in data entry
st.markdown("### 💷 Your Electricity Tariff")
st.markdown("Tell us who supplies your electricity, and we'll fill in your export rate (SEG) for you.")
...
# DEVELOPER NOTE: SEG rate logic - defaults to 4p for unknown suppliers
# This provides conservative estimates when supplier is not known
# Get SEG rate for selected supplier
if selected_supplier == "Other / Don't know":
    supplier_seg_rate = DEFAULT_SEG_RATE
    show_supplier_confirmation = False
    supplier_notes = None
elif selected_supplier != "Select your supplier...":
    supplier_seg_rate = get_seg_rate_for_supplier(selected_supplier)
    show_supplier_confirmation = True
    
    # Get supplier notes if available
    supplier_notes = None
    if supplier_df is not None:
        supplier_info = supplier_df[supplier_df['supplier'] == selected_supplier]
        if not supplier_info.empty:
            supplier_notes = supplier_info.iloc[0]['notes']
else:
    # No supplier selected yet - use default
    supplier_seg_rate = DEFAULT_SEG_RATE
    show_supplier_confirmation = False
    supplier_notes = None
...
# Show supplier notes under SEG rate field for better context
if supplier_notes and selected_supplier not in ["Select your supplier...", "Other / Don't know"]:
    st.caption(f"ℹ️ {selected_supplier}: {supplier_notes}")
else:
    st.caption("Your SEG rate is what your supplier pays you for exporting solar power to the grid.")

# Show default rate explanation when supplier not known
if selected_supplier in ["Select your supplier...", "Other / Don't know"]:
    st.caption("💡 If you're not sure who supplies your electricity, we've used a typical export rate of 4p/kWh.")

# Display supplier-specific SEG rate confirmation (only for known suppliers)
if show_supplier_confirmation and selected_supplier not in ["Select your supplier...", "Other / Don't know"]:
    st.success(f"✅ We've used {selected_supplier}'s SEG rate of {seg_rate}p/kWh (you can edit this above)")

st.write("")

# Clear heading for day/night rates
st.markdown("**💡 Enter your electricity rates (from your latest bill)**")
...
# Load CTA configuration from JSON
cta = schema.get("cta", {}) if schema else {}
cta_label = cta.get("label", "Calculate My Solar Savings")
cta_subtext = cta.get("subtext", "")

if st.button(cta_label, type="primary", use_container_width=True):
    
    # Check if schema is loaded
    if schema is None:
        st.error("Cannot display results without results_schema_min.json")
        st.stop()
    ...
    # Section title and subtitle from JSON
    st.markdown(f"## {schema['solar_summary']['title']}")
    st.markdown(schema['solar_summary']['subtitle'])
    
    # Display tagline if present
    if 'tagline' in schema['solar_summary']:
        st.markdown(f"*{schema['solar_summary']['tagline']}*")
    ...
    # Get metrics configuration from JSON
    metrics_config = schema['solar_summary'].get('metrics', {})
    
    with metric_col1:
        metric = metrics_config.get('annual_generation', {'label': 'Annual Generation', 'unit': 'kWh'})
        st.metric(
            metric['label'],
            f"{results['annual_generation']:,.0f} {metric['unit']}",
            help=schema['tooltips'].get('annual_generation', '')
        )
    
    with metric_col2:
        metric = metrics_config.get('use', {'label': 'Use (in your home)', 'unit': '%'})
        st.metric(
            metric['label'],
            f"{results['self_use_percent']*100:.0f}{metric['unit']}",
            help=schema['tooltips'].get('use', '')
        )
    
    with metric_col3:
        metric = metrics_config.get('save', {'label': 'Save (on bills)', 'unit': '£'})
        unit = metric['unit']
        st.metric(
            metric['label'],
            f"{unit}{results['savings']:,.0f}" if unit == '£' else f"{results['savings']:,.0f} {unit}",
            help=schema['tooltips'].get('save', '')
        )
    
    with metric_col4:
        metric = metrics_config.get('earn', {'label': 'Earn (sold back to grid)', 'unit': '£'})
        unit = metric['unit']
        st.metric(
            metric['label'],
            f"{unit}{results['export_income']:,.0f}" if unit == '£' else f"{results['export_income']:,.0f} {unit}",
            help=schema['tooltips'].get('earn', '')
        )
    ...
    with col_chart1:
        chart_config = schema['charts']['usage_vs_generation']
        st.markdown(f"**{chart_config['title']}**")
        
        comparison_data = {
            'Category': [chart_config['x_labels']['usage'], chart_config['x_labels']['solar']],
        ...
        fig_bar.update_layout(
            yaxis_title=chart_config['y_label'],
            showlegend=False,
            height=350,
    ...
    with col_chart2:
        chart_config = schema['charts']['self_use_vs_export']
        st.markdown(f"**{chart_config['title']}**")
        
        energy_split = {
            'Type': [chart_config['legend']['self_use'], chart_config['legend']['export']],
    ...
    # Detail section
    detail_config = schema['solar_detail']
    with st.expander(detail_config['title'], expanded=False):
        st.caption("Here's how we calculated your savings, in case you want to look under the hood.")
        st.write("")
        ...
        with detail_col1:
            ...
            st.metric(fields['self_use_kwh'], f"{results['self_used_kwh']:,.0f}")
            st.caption("Used in your home")
            st.metric(fields['self_use_savings'], f"£{results['savings']:,.0f}")
        
        with detail_col2:
            st.metric(fields['install_cost'], f"£{results['install_cost']:,.0f}")
            if results['battery_install_cost'] > 0:
                ...
                st.caption("💰 Estimated installation cost (UK national average)")
            st.metric("Exported to Grid", f"{results['exported_kwh']:,.0f} kWh")
            st.metric(fields['export_earnings'], f"£{results['export_income']:,.0f}")
    ...
    summary_config = schema['solar_summary']['summary_block']
    st.markdown(f"### {summary_config['heading']}")
    
    # Build formatted summary with exact structure
    summary_text = f"""
Your **{num_panels}-panel system ({results['system_size_kwp']:.1f} kWp)** would generate around **{results['annual_generation']:,.0f} kWh per year** — roughly **{coverage_percent:.0f}%** of your current electricity use.

🔹 **Use:** {results['self_use_percent']*100:.0f}% of that power goes straight into your home (≈ {results['self_used_kwh']:,.0f} kWh).  
🔹 **Save:** That cuts your annual electricity bill by about **£{results['savings']:,.0f}**.  
🔹 **Earn:** The rest ({results['export_percent']*100:.0f}%) is **sold back to the grid**, earning you around **£{results['export_income']:,.0f}**.

💰 **Total annual benefit: £{results['total_annual_value']:,.0f}**  
💡 **Estimated installation cost: £{results['install_cost']:,.0f}** (UK average)  
🌤️ **Simple payback:** Around **{results['payback_years']:.1f} years** — after which your system keeps saving and earning, the equivalent of **£{results['total_annual_value']/365:.2f}–£{(results['total_annual_value']/365)*1.3:.2f} a day** in passive income.
"""
    
    st.success(summary_text)
    
    # Intelligent recommendations (keep existing hardcoded logic for now as not in JSON)
    ...
    # Assumptions note (keep existing detailed assumptions)
    with st.expander("ℹ️ Calculation Details & Assumptions"):
        ...
        **Key Assumptions:**
        ...
        - EV charging impact: {'+' + str(int((EV_MULTIPLIERS[st.session_state.usage_type]-1)*100)) + '% increase' if has_ev == 'Yes' else 'Not applicable'}
