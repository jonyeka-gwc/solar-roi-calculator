...
if st.button("Calculate My Solar Savings", type="primary", use_container_width=True):
    ...
    # DEVELOPER NOTE: Simplified breakdown section based on MVP user feedback
    # Goal: Make data accessible without overwhelming non-technical users
    # Collapsible by default - advanced users can expand for details
    with st.expander("📈 Your Solar in Detail", expanded=False):
        ...
        with detail_col1:
            ...
            st.metric("Solar Power Produced", f"{results['self_used_kwh']:,.0f} kWh ({results['self_use_percent']*100:.0f}%)")
        ...
        with detail_col2:
            ...
            st.metric("Sent Back to the Grid", f"{results['exported_kwh']:,.0f} kWh ({results['export_percent']*100:.0f}%)")
            st.metric("Money Earned from Exports", f"£{results['export_income']:,.0f}")
